...
const Login = ({ setIsLogin }) => {
  ...
  const registerSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("http://gentle-thicket-67896.herokuapp.com/users/register", {
        username: user.name,
        email: user.email,
  ...
  const loginSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("http://gentle-thicket-67896.herokuapp.com/users/login", {
        email: user.email,
        password: user.password,
      });
      setUser({ name: "", email: "", password: "" });
